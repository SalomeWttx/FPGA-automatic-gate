...
--      Fonctionnement:
--          Les lignes sont des sorties du FPGA. Les colonnes des entrées.
...
-- Quand l'info est envoyée, on met à '1' toucheDetectee. La détection d'une touche se fait sur front montant uniquement.
--
...
--
-- L'entité capteur se charge de mettre à '1' une seule ligne, change la ligne à '1', et lit les colonnes actives. 
-- capteur envoie alors le signal brut à l'entité antipic, sous forme d'un vecteur à 16 éléments (1 par touche du clavier)
-- Après le traîtement par antipic, le signal est envoyé à l'entité dfm, qui ne garde que les fronts montants de chaque élément du signal
-- Enfin, l'entité traducteur prend en entrée un signal vecteur 16 colonnes contenant les fronts montants du clavier, et donne en sortie le numéro de la touche en binaire (numeroTouche),
-- ainsi qu'un signal toucheDetectee qui vaut '1' pendant une période d'horloge si une nouvelle touche est pressée.
--
--
-- Dependencies: 
